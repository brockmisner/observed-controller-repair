...
export const terminalRunStates = ['SUCCEEDED', 'FAILED', 'MISSED', 'CANCELLED'];
...
export const taskSchema = z.object({
  key: z.string().regex(/^[a-zA-Z0-9_-]{1,60}$/), name:z.string().trim().min(1).max(100),
  time:z.string().regex(/^(?:[01]\d|2[0-3]):[0-5]\d$/), templateId:z.string().trim().min(1).max(200),
  ...
  firstDay:z.number().int().min(1).max(730).default(1), lastDay:z.number().int().min(1).max(730).default(730),
  expectedMinutes:z.number().int().min(1).max(180).default(15),
...
export function dailyPlan(start:string, days:number, zone:string, tasks:Task[], fromDay=1,toDay=days) {
  const result:Array<{dayNumber:number;slotKey:string;scheduledAt:Date;deadlineAt:Date;task:Task}>=[];
  ...
  return result.sort((a,b)=>+a.scheduledAt-+b.scheduledAt||a.slotKey.localeCompare(b.slotKey));
}
export function expectedTaskCount(days:number,tasks:Task[]) {return tasks.reduce((n,t)=>n+Math.max(0,Math.min(days,t.lastDay)-t.firstDay+1),0);}
