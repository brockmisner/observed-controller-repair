(function (root) {
  ...
  function markup(icon) {
    return `<div class="driving-heading"><div><span class="eyebrow">Road trip</span><h3>Driving</h3></div><button type="button" class="btn ghost icon-button" data-driving-action="fit" title="Fit route on map" aria-label="Fit route on map">${icon("crosshair")}</button></div>
      ...
      <section id="drivingRadioStatus" class="driving-evidence" hidden></section>
  ...
  function create(options) {
    ...
    function render() {
      ...
      hidden("drivingRadioStatus", !current?.radioLive);
      if ($("drivingRadioStatus") && window.ObservatoryRadioStatus) html("drivingRadioStatus", window.ObservatoryRadioStatus.markup(current.radioLive));
